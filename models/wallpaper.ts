...
export async function insertWallpaper(wallpaper: Wallpaper) {
  ...
  const res = await db.query(
    `INSERT INTO wallpapers 
        (user_email, img_description, img_size, img_url, llm_name, llm_params, created_at, bDownload, bDisplay, bHighprior) 
        VALUES 
...
export function formatWallpaper(row: QueryResultRow): Wallpaper | undefined {
  let wallpaper: Wallpaper = {
	...
	bDownload: row.bDownload,
	bDisplay: row.bDisplay,
	bHighprior: row.bHighprior,
  };
